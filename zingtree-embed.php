...
define( 'zingtree_embed_version', '1.0.4' );
...
function zingtree_embed_plugin_init() {
	...
	if ( ( float ) $wp_version < 4.6 ) {
		...
		load_plugin_textdomain( 'zingtree-embed', false, $language_dir );
	}

}

add_action( 'init', 'zingtree_embed_plugin_init' );
...
function zingtree_set_plugin_meta( $links, $file ) {
	...
	if ( strpos( $file, 'zingtree-embed.php' ) !== false ) {
		$links = array_merge( $links, array( '<a href="http://wordpress.org/support/plugin/zingtree-embed">' . __( 'Support', 'zingtree-embed' ) . '</a>' ) );
		$links = array_merge( $links, array( '<a href="http://www.artiss.co.uk/donate">' . __( 'Donate', 'zingtree-embed' ) . '</a>' ) );
	}
...
function zingtree_shortcode( $paras = '', $content = '' ) {
	...
	$error = false;
	if ( $id == '' ) { $error = __( 'No ID specified', 'zingtree-embed' ); }
	if ( $style != 'buttons' && $style != 'panels' ) { $error = __( 'The \'style\' should only be \'buttons\' or \'panels\'', 'zingtree-embed' ); }

	// If an error occurred output that

	if ( $error !== false ) {

		$content = '<p style="color: #f00">Zingtree Embed ' . __( 'Error', 'zingtree-embed' ) . ': ' . $error . '</p>';
